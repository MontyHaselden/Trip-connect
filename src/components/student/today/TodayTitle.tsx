...
export function TodayTitle(props: {
  ...
  return (
    <header className="shrink-0 overflow-hidden border-b border-zinc-200/80 pb-4 pt-1">
      <p className="text-[11px] font-medium uppercase tracking-widest text-zinc-500">
        {schoolName}
      </p>
      <h1 className="mt-1 break-words text-2xl font-bold leading-tight tracking-tight text-zinc-900">
        {tripName}
      </h1>
      <p className="mt-2 break-words text-lg font-semibold text-zinc-800">{dateLine}</p>
      {cityLabel ? (
        <p className="mt-0.5 break-words text-sm text-zinc-600">{cityLabel}</p>
      ) : null}
